...
export default function DialogDetail({ handleOpen, open, selectedSO, handleChanges }: InputProps) {
    ...
    const [selectedIdSOToLunas, setSelectedIdSOToLunas] = useState("");

    const handleOpenConfirmDialog = (id_SO: string) => {
        handleOpen();
        setSelectedIdSOToLunas(id_SO);
    ...
    const handleConfirmDelete = async () => {
        await handleLunas(selectedIdSOToLunas);
        setOpenConfirmDialog(false);
    };

    const handleLunas = async (id_SO: string) => {
        const token = getCookie("token");

        if (token) {
            const response = await axios.put(`${process.env.BACKEND_API}/setLunas/${id_SO}`, {
            }, {
                headers: {
        ...
        <>
            <div className="z-50">
                <ConfirmDialog
                    ...
                    head="Konfirmasi Pelunasan"
                    message="Apakah anda yakin ingin melunasi transaksi ini?"
                />
            </div>
            <Dialog
                size="lg"
                open={open}
                handler={handleOpen}
                className="bg-transparent shador-none z-0"
                placeholder="">
                    ...
                    <>
                        ...
                        <CardFooter placeholder="" className="flex justify-between">
                            ...
                            {selectedSO.status_terima === 0 ?
                                ...
                                :
                                selectedSO.balance_due !== 0 ?
                                <Button color="green" onClick={(e) => handleOpenConfirmDialog(selectedSO.id_SO)} placeholder="">Lunas</Button>
                                :
                                <div>
